...
server_loop(Connections) ->
  receive
    {connected, Socket} -> server_loop([Socket | Connections]);
    {disconnected, Socket} -> server_loop(list_without(Socket, Connections));
    {msg, Receiver, Data} ->
      sendout_message(list_without(Receiver, Connections), Data),
      server_loop(Connections);
    {close} -> ok
  end.

list_without(Item, List) ->
  lists:filter(fun(C) -> C /= Item end, List).

sendout_message(Connections, Msg) ->
  lists:map(fun(C) -> gen_tcp:send(C, Msg) end, Connections).
...
parse_message(Server, Client, Msg) ->
  case strip_eol(Msg) of
    ...
    "\\name " ++ Name -> gen_tcp:send(Client, "your name is: " ++ Name ++ "\r\n");
